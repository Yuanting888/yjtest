...
class KnowledgeBaseService:
    ...
    def query(
        ...
        user=None,
        enable_rewrite: bool = True,
        enable_precipitate: bool = True,
    ) -> Dict[str, Any]:
        """查询知识库，支持 LLM 生成回答 + 知识沉淀"""
        ...
        try:
            ...
            # 知识沉淀：异步触发，不阻塞主流程
            if enable_precipitate and self._should_precipitate(search_results, answer):
                top_score = max(
                    (s.get("similarity_score", 0) for s in search_results), default=0
                )
                try:
                    import threading
                    threading.Thread(
                        target=self.precipitate_knowledge,
                        args=(query_text, answer, search_results),
                        kwargs={"query_log": query_log, "user": user, "quality_score": top_score},
                        daemon=True,
                    ).start()
                except Exception as e:
                    logger.warning(f"知识沉淀线程启动失败: {e}")
    ...
    def _generate_answer(self, query: str, sources: List[Dict[str, Any]]) -> str:
        """用 LLM 基于检索结果生成回答，降级时返回拼接内容"""
        if not sources:
            return "抱歉，没有找到相关信息。"

        context = "\n\n---\n\n".join(
            [f"[来源 {i+1}]\n{s['content']}" for i, s in enumerate(sources[:5])]
        )
        ...
        try:
            ...
            response = llm.invoke([
                {
                    "role": "system",
                    "content": (
                        "你是知识库问答助手。根据提供的参考内容回答用户问题。\n"
                        "要求：\n"
                        "1. 只基于参考内容作答，不要编造信息\n"
                        "2. 回答简洁、准确、结构清晰\n"
                        "3. 如果参考内容不足以回答问题，明确说明\n"
                        "4. 适当引用来源编号（如[来源1]）"
                    ),
                },
                {
            ...
            logger.warning(f"LLM 生成回答失败，降级为拼接模式: {e}")
            # 降级：直接拼接检索内容
            fallback = "\n\n".join([s["content"] for s in sources[:3]])
            return f"基于查询「{query}」检索到的相关内容：\n\n{fallback}"

    def _should_precipitate(
        self, sources: List[Dict[str, Any]], answer: str, threshold: float = 0.65
    ) -> bool:
        """判断本次查询结果是否值得沉淀：有高分来源且回答有实质内容"""
        if not sources or not answer or len(answer) < 50:
            return False
        top_score = max((s.get("similarity_score", 0) for s in sources), default=0)
        return top_score >= threshold

    def precipitate_knowledge(
        self,
        query: str,
        answer: str,
        ...
        """
        知识沉淀：将高质量查询-回答对写回知识库向量存储（LLM Wiki 核心思路）。
        沉淀内容会被向量化并存入 Qdrant，后续检索时可直接命中，
        使知识库随使用持续积累，而非每次从原始文档重新检索。
        """
        from .models import KnowledgeSummary

        try:
            # 构造沉淀内容（Markdown 格式，结构化）
            source_refs = "\n".join(
                [
                    f"- {s['metadata'].get('title', '未知文档')} "
                    f"(相似度: {s.get('similarity_score', 0):.3f})"
                    for s in sources[:3]
                ]
            )
            content = (
            ...
            # 复用 VectorStoreManager 写入，不关联具体 Document 对象
            vector_ids = self.vector_manager.vector_store.add_documents([lc_doc])
            if vector_ids:
                summary.is_vectorized = True
                summary.vector_id = vector_ids[0]
                summary.save(update_fields=["is_vectorized", "vector_id"])

            logger.info(
                f"💡 知识沉淀成功: kb={self.knowledge_base.name}, "
                f"summary_id={summary.id}, vector_id={summary.vector_id}"
            )
            return True

        except Exception as e:
            logger.warning(f"知识沉淀失败（不影响主流程）: {e}")
            return False
    ...
    def multi_kb_search(
        ...
        """多知识库并发检索 + Query Rewrite + 结果融合 + 上下文扩展"""
        import concurrent.futures

        # Query Rewrite：用第一个知识库的 service 做改写（共用 LLM 配置）
        ...
        if enable_rewrite:
            ...
            if first_kb:
                try:
                    rewritten = KnowledgeBaseService(first_kb)._rewrite_query(query)
                    if rewritten:
                        queries_to_run.append(rewritten)
                        logger.info(f"multi_kb_search Query Rewrite: '{query}' → '{rewritten}'")
                except Exception as e:
                    logger.warning(f"multi_kb_search Query Rewrite 失败: {e}")

        all_results = []
        seen_keys: set = set()
        kbs = KnowledgeBase.objects.filter(id__in=knowledge_base_ids)

        def _search_one(kb, q):
            manager = VectorStoreManager(kb)
            results = manager.similarity_search(q, k=k, score_threshold=score_threshold)
            for r in results:
                r["metadata"]["knowledge_base_id"] = str(kb.id)
            return results

        with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(4, len(kbs) * len(queries_to_run))
        ) as executor:
            futures = {
                executor.submit(_search_one, kb, q): (str(kb.id), q)
                for kb in kbs
                for q in queries_to_run
            }
            for future in concurrent.futures.as_completed(futures):
                kb_id, q = futures[future]
                try:
                    ...
                    logger.warning(f"知识库 {kb_id} 检索失败 (query='{q}'): {e}")

        all_results.sort(key=lambda x: x.get("similarity_score", 0), reverse=True)
        all_results = VectorStoreManager._mmr_diversify(all_results, k)

        # 上下文扩展（复用第一个 kb 的 service 实例）
        if kbs:
            try:
                svc = KnowledgeBaseService(kbs.first())
                all_results = svc._expand_context(all_results)
            except Exception as e:
                logger.warning(f"multi_kb_search 上下文扩展失败: {e}")
