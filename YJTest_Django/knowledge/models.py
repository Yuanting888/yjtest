...
class KnowledgeSummary(models.Model):
    """
    知识沉淀模型（LLM Wiki 思路）
    将高质量的查询-回答对沉淀为结构化知识，写回知识库向量存储，
    使知识库随使用不断积累，而非每次从原始文档重新检索。
    """
    SOURCE_CHOICES = [
        ('query_answer', '查询问答沉淀'),
    ...
    title = models.CharField(_('摘要标题'), max_length=200)
    content = models.TextField(_('沉淀内容'))
    source = models.CharField(
        _('来源类型'), max_length=30, choices=SOURCE_CHOICES, default='query_answer'
    )
    # 关联的原始查询日志（可选）
    query_log = models.ForeignKey(
        ...
        related_name='summaries',
        verbose_name=_('来源查询日志')
    )
    # 是否已向量化写回 Qdrant
    is_vectorized = models.BooleanField(_('已向量化'), default=False)
    vector_id = models.CharField(_('向量ID'), max_length=100, blank=True, null=True)
    # 质量评分（0-1），用于过滤低质量沉淀
    quality_score = models.FloatField(_('质量评分'), default=1.0)
