...
from .models import KnowledgeBase, Document, DocumentChunk, QueryLog, KnowledgeSummary
...
class KnowledgeSummaryAdmin(admin.ModelAdmin):
    list_display = ['title', 'knowledge_base', 'source', 'quality_score', 'is_vectorized', 'created_at']
    list_filter = ['source', 'is_vectorized', 'knowledge_base', 'created_at']
    search_fields = ['title', 'content', 'knowledge_base__name']
    readonly_fields = ['id', 'vector_id', 'is_vectorized', 'created_at', 'updated_at']

    fieldsets = (
        ('基本信息', {
            'fields': ('knowledge_base', 'title', 'source', 'quality_score', 'creator')
        }),
        ('沉淀内容', {
            'fields': ('content',)
        }),
        ('向量化状态', {
            'fields': ('is_vectorized', 'vector_id'),
            'classes': ('collapse',)
        }),
        ('关联信息', {
            'fields': ('query_log',),
